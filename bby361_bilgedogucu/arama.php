<?php
    ...
    SELECT 
        ...
        eser.eserAdi, 
        eser.eserBasimYeri, 
        eser.eserBasimTarihi, 
        eser.eserSayfaSayisi, 
        eser.eserDili,
        yazar.yazarAdi, 
        yazar.yazarSoyadi, 
        yazar.yazarDogumTarihi, 
        yazar.yazarCinsiyeti
    FROM 
        eser
    INNER JOIN 
        yazar ON eser.yazarID = yazar.yazarID
    Where 
    eser.eserAdi LIKE '%$q%' OR
    yazar.yazarAdi LIKE '%$q%'
");
    // $sorgu = $mysqli->query($sorgu);
    $satir = mysqli_fetch_assoc($sorgu);
    // echo '<br><br><br><br><br>';
    // echo '<pre>'; print_r($satir); echo '</pre>';
    ...
    <h2>Eser Kayıtları</h2>
      ...
      <p>
          ...
          for($i = 0; $i < $toplam; $i++) {
              ...
              printf("<p><b>Yazar Adı:</b> ".$satir["yazarAdi"]." ".$satir["yazarSoyadi"]."</br>");
